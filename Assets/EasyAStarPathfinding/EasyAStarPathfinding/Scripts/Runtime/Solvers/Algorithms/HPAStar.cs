...
{
    public class HPAStar<T> : ISolver<T> where T : IGraphNode
    {
        public PathfindingState state { get; protected set; }

        private readonly List<T> result = new List<T>();

        internal readonly PathfindingContextHierarchical<T> context;
        internal T startingNode;
        internal T endingNode;
        private int curLevel;

        public HPAStar(IHierarchicalGraphData<T> graphData, Algorithms algorithm)
        {
            this.context = new PathfindingContextHierarchical<T>(graphData, algorithm);
